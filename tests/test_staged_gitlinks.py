...
# --- 五、过闸门的非空基线 --------------------------------------------------
#
# 第四节那三条是**判据级**的：直接调 staged_gitlinks，没经过闸门。第三节过了
# 闸门，但基线是空集。两边合起来仍留一个缺口 —— **闸门条件本身**在非空基线上
# 从没被测过。实测 M118 有两条变异因此存活（950 全绿）：
#
#   count-only   `len(after) > len(before)`    条目数不变的重指向漏过去
#   exists-only  `if staged_gitlinks(ws):`     本来有子模块的仓库每轮判红
#
# 杀掉它们各需要一条测试，且两条都必须真跑 Dispatcher。


def _wt_with_gitlink(tmp_path: Path, name: str = "vendor") -> Path:
    """基线里**已经合法拥有**一条 gitlink 的 worktree。

    worktree 里 `vendor/` 是个空目录（子模块没 init），攻击方因此可以在里面
    `git init` 造一个历史完全不同的内层仓库 —— 这就是「重指向」的真实形状。
    """
    root = _repo(tmp_path)
    _stage_gitlink(root, name)
    _git(root, "commit", "-qm", "add vendor")
...
def _dispatch_cb(tmp_path: Path, ws: Path, checks_ran: list, attack=None):
    """和 _dispatch 同形，但攻击动作由调用方给。只动第一轮，理由同上。"""
    from factory.audit.models import SupervisorRole, Verdict
    from factory.audit.store import AuditStore
    from factory.dispatcher import Dispatcher
    from factory.supervisors.base import SupervisorReport
    ...
    return d.run(
        Task(task_id="T-repoint", prompt="改点东西",
             checks=(CheckSpec(name="ok", command="true"),)),
        ws,
    )
...
def _repoint(ws: Path, name: str = "vendor") -> None:
    """在 worktree 的空 gitlink 目录里造个新仓库并 add —— 路径不变、哈希变。"""
    _inner_repo(ws, name)
    _git(ws, "add", name)
