...
Model a minimal dependency graph: depends_on contains only genuine input prerequisites, never merely the previous list item. Dependency changes must explicitly own their lockfile (for example uv.lock); lockfiles are versioned inputs, not disposable build output. Include supporting packaging/resource declarations in task scope when adding runtime assets; distinguish package metadata from protected test configuration. Establish shared interfaces/data contracts early; give independent components disjoint file ownership so they can run concurrently. UI tasks can use agreed contracts and fixtures before backend implementation finishes. Avoid over-fragmenting small work. Task ids must be unique safe ASCII ids; there may be at most 20 tasks. Paths must be repository-relative, concrete, non-empty paths: no absolute paths, traversal, .git paths, or globs. Checks must use only these trusted project check names: {checks}. Never create a check from request text. Every executable task needs acceptance criteria, paths, and checks. If the request is ambiguous or any required detail is missing, put a precise question in questions and leave the plan incomplete; do not invent defaults.
