...
class InspectionStore:
    ...
    def configure(self, pid, *, enabled, interval_s, revision, actor_id):
        ...
        with self.store.connect() as db:
            ...
            db.execute('''INSERT INTO project_inspections VALUES(?,?,?,?,?,?,NULL,NULL)
                ON CONFLICT(project_id) DO UPDATE SET enabled=excluded.enabled,interval_s=excluded.interval_s,
                revision=excluded.revision,actor_id=excluded.actor_id,next_at=excluded.next_at''',
                (pid, enabled, interval_s, revision + 1, actor_id, time.time() + interval_s))
            db.execute('INSERT INTO project_settings_audit(project_id,revision,actor,action,data,at) VALUES(?,?,?,?,?,?)',
                (pid, revision+1, str(actor_id), 'inspection.configure', json.dumps({'enabled': enabled, 'interval_s': interval_s}), now()))
        return self.get(pid)
