...
[实践说明](PROJECT-AGENT.zh-CN.md)。全新面板和运行环境管理见 [新版工作台](WORKBENCH.zh-CN.md)。
...
三家 SDK 是可选依赖，这份完整安装命令会安装全部 SDK 与配套运行时；按需安装时也必须保留所用 extras。后续用 `.venv/bin/` 直接启动，避免普通 `uv run` 重新同步并卸载 extras。版本记录在 `uv.lock`；真实安装接口核对版本为 Codex 0.147.0、Claude 0.2.152、DSH 0.1.2rc1。SDK 安装成功不表示账号或指定模型已获得权限。
...
.venv/bin/factory-web create-user owner
.venv/bin/factory-web serve
```
...
本次开发使用 Luna 子代理，并不意味着你的供应商账号存在名为 Luna 的可调用模型。四个档位在首次启动时从环境变量初始化，此后以“运行配置”页保存到数据库的版本为准。环境变量变更不会覆盖已保存的设置。每次规划冻结配置，后续审批和执行使用同一版本。未填写模型时会明确停止，不会自动猜型号。

## 3. 第一次运行

1. 打开配置的域名并登录；本地默认 `http://127.0.0.1:8788`。先进入“运行配置”，查看安装诊断、填写真实模型 ID，再按需点击可能计费的连接测试。
2. 登记服务器上已有的 Git 仓库。路径必须是 `FACTORY_WORKSPACE_ROOT` 的独立子目录，基线必须是存在的本地分支。
3. 在检查编辑器逐项配置名称、程序与参数。目标仓库依赖须事先安装；例如程序为项目环境的 Python，参数依次为 `-m`、`pytest`、`-q`。检查命令由工程师维护；模型只能引用检查名称。
...
当前没有自动修复失败任务、续跑剩余 DAG 或按价格表估算 Codex/DSH 成本。实际美元未知时不会把它记为零：默认停止下一批派发。操作者可以在运行配置中明确选择“按执行限制继续”，同时限制整次执行时长、任务数和并发数；仍标记费用不完整并禁止自动发布。策略改变只影响新计划；已有计划需要补充需求重新规划。预算是依据已报告成本停止后续派发的阈值，不是供应商侧硬限额。
...
`users.db` 保存账号与会话，`control.db` 保存项目、项目设置审计、模型配置版本、连接测试记录、计划、运行和追加写事件。备份前停止服务，同时保留数据库及可能存在的 WAL 文件；备份与工作区也应限制文件权限。
...
2. 在项目页的“项目代理”中打开“档案/知识”维护工程使命、约束和知识条目。`active` 是可进入规划上下文的已审核记忆；`candidate` 只供人工复核，不能当成事实。更新使用 revision CAS，历史版本和 provenance 保留。
