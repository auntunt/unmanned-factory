...
`RuntimeSettings(store, profiles=None, limits=None)` SQLite persistence with immutable versions/audit and CAS. `get()` -> `{revision,profiles,limits,updated_at}`. Defaults from provided profiles (else FACTORY_* environment) and limits `{timeout_s:600,max_parallel:2,max_tasks:20,unknown_cost_policy:'stop'}`. `update(data,expected_revision,actor)` validates complete four-role mapping (`planner,cheap,standard,strong`), provider in claude/codex/dsh, model string <=160 (empty means incomplete config), timeout30..1800, parallel1..4, max_tasks1..20, policy stop/allow_bounded. Reject DSH as planner. No credentials/URLs/paths/commands in writable schema. ValueError->400, Conflict->409. Get preserves existing config across process restart; environment only seeds initial config.
