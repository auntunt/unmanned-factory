...
- ~~适配场景还没有用真实模型跑过~~ —— **已作废**：批次九用同一条中转站链路跑通了
  接口适配切片（编码是真实模型、第三方是本机模拟端），证据见
  [EVIDENCE-real-model.md](EVIDENCE-real-model.md) 第二节。
- 维护与信创两个面还**没有各自的「回答澄清问题」入口**：视图现在可读、能看到
  阻塞原因，但要答问题目前只有适配面有 `clarify`。
