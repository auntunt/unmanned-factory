...
2. 生产部署目标登记为空。
3. ~~`FACTORY_GITHUB_TOKEN` / `FACTORY_DEPLOY_KEY_DIR` 未配置。~~ **本条错误，2026-09-19 由 Codex 现场检查更正：生产上这两个环境变量均已配置（Codex 只输出布尔值，未读取值）。原文把本机环境观察错误地当成了生产状态。** `deploy_targets` 仍为 0，固定测试地址仍缺，这两条不变。
...
# 附：Codex 对 `4d2e3f8` 的独立验收结论（2026-09-19，暂不通过）

Codex 复现了三项 P1 阻塞，三条最小复现均红，文件 `docs/acceptance/webuddy-next-2026-09-19/test_codex_review.py`：

1. **会话 Skill 只有登记，没有实际装载**：`session_skills.py` 只存元数据，ZIP 正文与 GitHub 内容都没保存；无项目 do 聊天只装载角色 manifest；`session_skill_snapshot` 全仓只有冻结与展示消费者，**没有执行装载消费者**。导入 201、聊天 completed，但规则对模型不可见。
2. **会话 Skill 读取缺少访问授权**：`_verify_session` 只 `SELECT 1` 查存在性，无关 member `GET /api/v4/sessions/{sid}/skills` 返回 200 及完整元数据。**这是越权读取。**
3. **配置对话完成后永久 pending**：完成只追加 completed 消息而不结束原 pending，前端轮询不停、`onConfigChanged` 不触发。

**主会话对自身验收的检讨**：上一轮 V1 真实服务器核查报 6/6 通过，但它验的是「跨会话删除返回 404」，**没有验「无关用户跨用户读取」**——第 2 条是被 V1 的测试设计漏掉的，不是 V1 撒谎。第 1 条同理：V1 验了绑定存在与重启保留，没有验「正文真的到了模型手里」。**绑定表有行 ≠ 能力被用上**，这是本轮验收设计的教训。

Codex 现场已通过：原有定向 49 项、Linux 隔离与工具相关 13 项、工具执行包下载 API→hash/版本→平台外 CLI 校验 1 项。

修复任务单：`tasks/R1.md`（正文装载）、`tasks/R2.md`（会话授权）、`tasks/R3.md`（作业状态）。`paused` 独立状态继续后置，不扩本轮范围。
