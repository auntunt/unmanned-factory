...
rid=json.loads((BASE/'v1.json').read_text())['id'];old=api('GET','/api/v2/runs/'+rid);assert old['status'] in ('needs_human','cancelled');save('v1-final-stopped',old)
...
review_ref='refs/heads/acceptance/closure-v1-review-20260920'
...
save('review-source-upload',{'repository':repo,'commit':sha,'ref':review_ref,'source_run_id':rid,'purpose':'candidate source upload; not passed release or platform publish'})
...
cfg={'repository':repo,'commit':sha,'run_id':rid};save('v1-pinned',cfg);subprocess.run(['sudo','-n','install','-m','644',str(BASE/'v1-pinned.json'),'/opt/webuddy-demo-counter/target.json'],check=True)
body={'project_id':p['id'],'operation':'release','execute_deploy':True,'interaction_mode':'automatic','idempotency_key':'closure-release-candidate-v1-20260920','request':'审核并部署当前分支已有的 Python 标准库 Counter 网页服务。这是已有候选的独立部署验收，不是新增开发。不要改任何文件或重构，不新增文档/功能/测试。运行已有 unittest，启动现有 app.py，通过HTTP核验 /health 为 ok=true，/version 从 WEBUDDY_COMMIT 返回部署版本，POST /api/counter 的 increment/decrement/reset 正确；核对现有 Dockerfile 非root、python:3.12-alpine、8080端口。候选源码已上传 GitHub 审核分支，固定部署提交 '+sha+'。本阶段不要求浏览器视觉验收，不要编写模拟代理或重复截图：公网反代页面由Codex在部署后实际操作验收。通过后由平台调用已绑定目标的受控 deploy/health_check/service_status；不要自行SSH或Docker。用户已授权本次示例容器发布。只返回真实检查证据与本阶段验收结果，若缺项明确指出。'}
r=api('POST','/api/v2/runs',body);save('release-v1',r);print(json.dumps({'release_run_id':r['id'],'candidate_commit':sha,'review_ref':review_ref}))
