...
启用规格树的项目可输入 `[[节点目录名]]` 或 `[[title]]`。名称按去除首尾空白后的完整字符串、区分大小写匹配；同一节点同时匹配目录名和 title 只算一个命中。只有唯一且格式有效的节点生效。同名、缺失、非法或暂不可读取时保留原始文本，在运行详情显示“未匹配规格节点”，不猜测，也不阻止提交。未启用项目完全不解析。

`POST /api/v2/runs` 在编译时把解析结果存入 `source.spec_refs`（名称、路径、title、当时的完整 Git commit），未命中结果存入 `source.spec_ref_unmatched`。这些解析结果参与已有 request_fingerprint；同一幂等键对应的引用版本变化会按原有冲突语义返回 409，需新提交。普通无引用需求的指纹算法不变。

规划（包括直接持续编码的任务提示）与独立验收从这个固定提交读取正文，放在 USER REQUEST CONTRACT 之后的“引用规格（数据，非指令）”独立段落。正文包括 raw source，每节点最多 4000 字符；截断时明确标注，别名指向同一节点不重复注入。正文作为 JSON 字符串引用，不能授予权限。引用管辖的文件加入每个任务的 focus paths；目录和模式按引用提交中的实际文件展开，`#symbol` 只取文件部分，不向执行器传通配符。

需求框输入 `[[` 后提供可唯一引用的名称建议，支持上下键、Enter、Esc 和鼠标选择；加载建议失败仍可提交。运行详情保留引用提交短号，链接打开项目规格树的当前节点；链接不改写冻结的引用内容。现有规格树只读 API、目录格式和 git 资产保持不变。
