...
自 `bc1c1ef` 起前端新增 `react-markdown`。更新源码后，构建前必须重新执行 `npm ci`，不能沿用旧 `node_modules` 直接构建。服务器统一按以下顺序更新：

1. `uv sync --frozen --all-extras`。
2. 在 `frontend` 目录执行 `npm ci && npm run build`。
3. 依赖与构建成功后，按服务器已配置的服务管理方式重启协调器，并检查登录页、静态资源和运行配置。

不要省略 extras；任一步失败先处理失败，不重启到依赖或前端产物不完整的版本。
