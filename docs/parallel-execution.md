...
Untracked virtualenv interpreter symlinks are normal disposable environment output and are excluded with the virtualenv. This does not exempt tracked symlink changes or unexpected symlinks in egg-info metadata. Recovery must apply the same generated-output classification as execution so a valid saved draft does not become unrecoverable merely because tests created a virtualenv.
