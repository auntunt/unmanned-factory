...
describe('MonitorPage 项目行下钻', () => {
  ...
  it('项目名与下一步在首列可见并链接到仓库详情，状态徽标不是链接；嵌入前缀下仍留在嵌入路由', async () => {
    ...
    for (const base of ['/maintenance', '/embed/maintenance']) {
      ...
      expect(screen.getByRole('link', { name: '查看待补充项' }).closest('td')).toBe(name.closest('td'))
      expect(screen.getByRole('columnheader', { name: '代码库与下一步' })).toBeTruthy()
