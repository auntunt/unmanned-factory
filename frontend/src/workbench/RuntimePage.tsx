...
import ServerTargets from './ServerTargets'
...
const defaultLimits: RuntimeLimits = { timeout_s: 14400, max_parallel: 2, max_tasks: 20, unknown_cost_policy: 'allow_bounded' }
...
function normalizeConfiguration(value: Partial<RuntimeInspection> | null | undefined): RuntimeInspection {
  ...
  const configuration: RuntimeInspection = {
    ...
    limits: {
      ...
      unknown_cost_policy: sourceLimits.unknown_cost_policy === 'stop' ? 'stop' : 'allow_bounded',
    },
...
function validateDraft(draft: RuntimeConfiguration): string | null {
  ...
  if (draft.limits.timeout_s < 30 || draft.limits.timeout_s > 86400) return '超时必须在 30 秒至 24 小时之间。'
...
export default function RuntimePage({ csrfToken, onUnauthorized }: PageProps) {
  ...
  if (loading && !inspection) return <div className="wb-detail-page"><LoadingCard label="正在读取运行配置与环境诊断" /></div>
  if (!inspection) return <div className="wb-detail-page">{error ? <ErrorNotice message={error} /> : <EmptyState title="运行配置暂不可用" description="服务端没有返回运行配置。请稍后重试。" />}</div>
  return <div className="wb-detail-page"><PageHeader title="运行配置" description="配置模型角色、任务边界和执行前的环境检查。读取诊断不会发起付费请求。" actions={<button className="wb-button wb-button-primary" disabled={saving} onClick={() => void save()}>{saving ? '保存中…' : '保存设置'}</button>} />{error && <ErrorNotice message={error} />}{notice && <div className="wb-notice" role="status">{notice}</div>}<div className="wb-runtime-grid"><div><section className="wb-detail-card wb-form" aria-labelledby="profiles-title"><h2 id="profiles-title">模型角色</h2><p className="wb-runtime-note">四个角色必须分别配置。provider 与模型 ID 来自你的已安装账户，页面不会替你猜测有效模型。</p><div className="wb-profile-grid">{RUNTIME_ROLES.map((role) => { const profile = draft.profiles[role]; return <div className="wb-profile-row" key={role}><label htmlFor={`runtime-profile-${role}-provider`}>{runtimeRoleLabel(role)}</label><select id={`runtime-profile-${role}-provider`} aria-label={`${runtimeRoleLabel(role)} provider`} value={profile.provider} onChange={(event) => updateProfile(role, 'provider', event.target.value)}><option value="">选择 provider</option>{RUNTIME_PROVIDERS.map((provider) => <option value={provider} disabled={role === 'planner' && provider === 'dsh'} key={provider}>{runtimeProviderLabel(provider)}{role === 'planner' && provider === 'dsh' ? '（规划不可用）' : ''}</option>)}</select><input id={`runtime-profile-${role}-model`} aria-label={`${runtimeRoleLabel(role)}模型 ID`} value={profile.model} maxLength={160} onChange={(event) => updateProfile(role, 'model', event.target.value)} placeholder="填写账户中的模型 ID" /></div> })}</div></section><section className="wb-detail-card wb-runtime-section" aria-labelledby="limits-title"><h2 id="limits-title">执行边界</h2><p className="wb-runtime-note">供应商计费由中转站管理；本地停止线在项目设置，月度 token 额度在团队页。这里设置任务数、并行数和超时。</p><div className="wb-runtime-limits"><div className="wb-field"><label htmlFor="runtime-timeout-s">单阶段最长运行时间（小时）</label><input id="runtime-timeout-s" type="number" min={0.01} max={24} step={0.01} value={draft.limits.timeout_s / 3600} onChange={(event) => updateLimit('timeout_s', String(Math.round(Number(event.target.value) * 3600)))} /><span className="wb-runtime-note">默认 4 小时，最长 24 小时。执行阶段包括全部子任务与修复；调高后继续暂停任务即可生效。</span></div><div className="wb-field"><label htmlFor="runtime-max-parallel">单次运行并行任务</label><input id="runtime-max-parallel" type="number" min={1} max={4} value={draft.limits.max_parallel} onChange={(event) => updateLimit('max_parallel', event.target.value)} /></div><div className="wb-field"><label htmlFor="runtime-max-tasks">单次最大任务数</label><input id="runtime-max-tasks" type="number" min={1} max={20} value={draft.limits.max_tasks} onChange={(event) => updateLimit('max_tasks', event.target.value)} /></div></div></section></div><div><AdminConfigChat csrfToken={csrfToken} onUnauthorized={onUnauthorized} onConfigChanged={() => void load()} /><OperationsSettings csrfToken={csrfToken} onUnauthorized={onUnauthorized} /><ServerTargets csrfToken={csrfToken} onUnauthorized={onUnauthorized} /><ToolDiagnostics inspection={inspection} /><ReadinessSummary inspection={inspection} />{hasBlockers && <section className="wb-detail-card wb-runtime-section"><h2>当前阻塞项</h2><ul className="wb-tool-issues">{inspection.blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul></section>}<LiveProbes inspection={inspection} unsaved={unsaved} probing={probing} onProbe={(role) => void probe(role)} /></div></div></div>
}
