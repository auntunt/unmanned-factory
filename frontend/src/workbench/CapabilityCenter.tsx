...
export default function CapabilityCenter(props: PageProps) {
  const [params] = useSearchParams(); const tab = params.get('tab') === 'capabilities' ? 'capabilities' : 'modules'
  ...
  return <div className="wb-page wb-ability-center"><PageHeader title="能力中心" description="搭配工作方法，复用运行中沉淀的经验。" /><nav className="wb-ability-tabs" role="tablist" aria-label="能力分类" onKeyDown={tabKeys}>{(['modules','capabilities'] as const).map(key => <Link role="tab" id={`ability-tab-${key}`} aria-controls="ability-panel" aria-selected={tab === key} tabIndex={tab === key ? 0 : -1} key={key} to={capabilityHref(key)}>{key === 'modules' ? '方法模块' : '沉淀能力'}</Link>)}</nav><ProvenanceContext.Provider value={provenance}><section role="tabpanel" id="ability-panel" aria-labelledby={`ability-tab-${tab}`}>{tab === 'modules' ? <ModulesPage {...props} embedded /> : <CapabilitiesPage {...props} embedded />}</section></ProvenanceContext.Provider></div>
}
