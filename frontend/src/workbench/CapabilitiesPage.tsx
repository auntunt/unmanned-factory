...
import { PromoteSkill } from './AgentEvolution'
import { CapabilitySources } from './capability-links'
...
function CapabilityCard({ capability, selected, onSelect }: { capability: Capability; selected: boolean; onSelect: () => void }) {
  return <article className="wb-capability-record"><button type="button" className={`cl-capability-card ${selected ? 'is-selected' : ''}`} onClick={onSelect} aria-pressed={selected}>
    ...
    <small>v{capability.revision} · 更新于 <ListTime value={capability.updated_at} /></small>
  </button><CapabilitySources capability={capability} /></article>
}

export default function CapabilitiesPage({ csrfToken, onUnauthorized, user, embedded = false }: PageProps & { embedded?: boolean }) {
  ...
  const setSearchParams = (next: Record<string,string>, options?: { replace?: boolean }) => writeSearchParams({ ...next, ...(embedded ? { tab: 'capabilities' } : {}) }, options)
  ...
  return <div className="cl-page">
    {embedded ? <div className="wb-form-actions">{isAdmin && <button className="cl-button cl-button-primary" onClick={() => setShowNew(value => !value)}>{showNew ? '关闭新建' : '新建工作能力'}</button>}</div> : <PageHeader title="经验库" description="保留已有工作模板与验收经验，逐步完善可复用的做法。" actions={isAdmin ? <button className="cl-button cl-button-primary" onClick={() => setShowNew((value) => !value)}>{showNew ? '关闭新建' : '新建工作能力'} <span aria-hidden="true"><Icon name="plus" /></span></button> : undefined} />}
