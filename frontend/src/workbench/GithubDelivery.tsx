...
export default function GithubDelivery({ run, csrfToken, onUnauthorized, isAdmin, configured }: { run: Run; csrfToken: string; onUnauthorized: () => void; isAdmin: boolean; configured: boolean }) {
  ...
  if (run.status === 'published' || published) return <span role="status">{githubPublicationLabel(displayedRun, configured)} {link && <a className="wb-button" target="_blank" rel="noreferrer" href={link}>打开仓库 <Icon name="external" /></a>}</span>
  return <div>
