...
export default function RunsPage({ onUnauthorized }: PageProps) {
  ...
  const visible = useMemo(() => (runs ?? []).filter((run) => {
    const matchesFilter = filter === 'all' || filter === 'active' ? (filter === 'all' || ['requirement_analysis', 'received', 'planning', 'queued', 'running', 'verifying', 'publishing'].includes(run.status)) : filter === 'attention' ? !run.retry_run_id && !run.inspection_superseded_by && ['awaiting_spec_confirmation', 'needs_clarification', 'awaiting_approval', 'needs_human', 'inspection_failed'].includes(run.status) : filter === 'failed' ? ['failed', 'discarded', 'cancelled'].includes(run.status) : run.status === filter
