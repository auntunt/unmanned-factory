...
export default function RunsPage({ onUnauthorized }: PageProps) {
  ...
  }).sort((a, b) => String(b.updated_at).localeCompare(String(a.updated_at))), [filter, projects, query, runs])
  ...
  return <div className="wb-page">
    ...
    {runs && visible.length === 0 && <div className="wb-card"><EmptyState title={query ? '没有匹配的运行' : '还没有运行'} description={query ? '换一个关键词或状态筛选试试。' : '从项目提交一条需求后，运行现场会出现在这里。'} action={!query ? <Link className="wb-button wb-button-primary" to="/projects">浏览项目</Link> : undefined} /></div>}
    {runs && visible.length > 0 && <section className="wb-card wb-run-table-card"><div className="wb-table-wrap"><table className="wb-table wb-runs-table"><thead><tr><th>需求</th><th>项目</th><th>状态</th><th>计划</th><th>更新时间</th><th /></tr></thead><tbody>{visible.map((run) => { const project = projectNames.get(String(run.project_id)); return <tr key={String(run.id)}><td><Link className="wb-table-link" to={`/runs/${encodeURIComponent(String(run.id))}`}>{run.plan?.title || run.request.slice(0, 100)}</Link><small className="wb-mono">#{run.id}</small></td><td>{project ? <Link className="wb-project-inline" to={`/projects/${encodeURIComponent(String(project.id))}`}>{project.name}<small className="wb-mono">#{String(project.id).slice(0, 8)}</small></Link> : <span className="wb-project-inline"><span>项目暂未返回</span><small className="wb-mono">#{String(run.project_id).slice(0, 8)}</small></span>}</td><td><StatusBadge status={run.status} /></td><td>{run.revision ? `v${run.revision}` : '—'}</td><td>{formatDate(run.updated_at)}</td><td><Link className="wb-row-arrow" to={`/runs/${encodeURIComponent(String(run.id))}`} aria-label="查看运行">→</Link></td></tr> })}</tbody></table></div></section>}
  </div>
}
