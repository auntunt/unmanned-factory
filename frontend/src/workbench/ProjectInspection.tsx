...
export default function ProjectInspection({ projectId, csrfToken, onUnauthorized, isAdmin }: { projectId: string; csrfToken: string; onUnauthorized: () => void; isAdmin: boolean }) {
  ...
  return <section className="wb-card wb-inspection-card"><div className="wb-inspection-head"><h2>定时巡检</h2><details className="wb-help"><summary aria-label="巡检帮助"><Icon name="triangle" className="wb-disclosure-icon" />? 巡检说明</summary><p>只检查项目的隔离副本并记录结果，不改代码、不部署；首次巡检在一个间隔后入队。</p><label className="wb-checkbox"><input type="checkbox" checked={config?.remote_read_only ?? false} disabled={!config || !isAdmin || busy} onChange={e => config && void save(config.enabled, config.interval_s, e.target.checked)} />远程只读探测（默认关闭）</label><p>开启后对绑定服务器执行已登记的只读检查。</p></details></div>
    ...
    {config.last_result && <details className="wb-help"><summary><Icon name="triangle" className="wb-disclosure-icon" />结果摘要</summary><p>{config.last_result}</p></details>}
