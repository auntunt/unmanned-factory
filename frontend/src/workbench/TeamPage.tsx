...
function TeamPage({ csrfToken, onUnauthorized, user }: PageProps) {
  ...
  return <div className="tm-page">
    <PageHeader title="团队" description="管理成员、项目权限与本地 token 用量。额度留空时仅监测；供应商账户余额仍由中转站管理。" actions={<button className="tm-button tm-button-secondary" onClick={() => void load()} disabled={loading}>{loading ? '更新中…' : '刷新'}</button>} />
    ...
    {data && <>
      {<div className="tm-context"><strong>{data.month} · {data.timezone}</strong><span>计量从 {data.tracking_started_at ? formatDate(data.tracking_started_at) : '治理启用后'} 开始</span></div>}
      {<details className="tm-method-note"><summary><Icon name="triangle" className="wb-disclosure-icon" />额度如何计算</summary><p>成员和项目共享运行记录；额度按每月 UTC 重置，统计模型输入与输出 tokens（含缓存输入）。额度是调度门槛，不是美元金额或供应商订阅 credits；单次执行可能超过预留值，待核对占用会保留到管理员核对。</p></details>}
      {displayQuota && <QuotaSummary quota={displayQuota} label={isAdmin ? '工作区本月用量' : '我的本月用量'} />}
      ...
      {<section className="tm-card"><div className="tm-card-head"><div><span className="tm-kicker">项目额度</span><h2>项目用量与上限</h2><p>项目额度是调度门槛，项目记录仍由小组共享。</p></div></div><div className="tm-project-list">{data.projects.map((project) => <ProjectQuotaRow key={project.id} project={project} isAdmin={isAdmin} busy={busy} onQuota={updateQuota} />)}</div></section>}
      {<section className="tm-card"><div className="tm-card-head"><div><span className="tm-kicker">调用记录</span><h2>调用与待核对占用</h2><p>未知调用保留在额度中；这里同时展示已预留和已结算记录，便于追溯。</p></div><span className="tm-count">{unknownCalls.length} 笔用量待核对</span></div>{calls.length === 0 ? <p className="tm-empty-line">当前没有调用记录。</p> : <div className="tm-call-list">{calls.slice(0, 100).map((call) => <CallRow key={call.id} call={call} isAdmin={isAdmin} busy={busy} onReconcile={reconcile} />)}</div>}</section>}
      {<details className="tm-card tm-control-card"><summary className="tm-card-head"><Icon name="triangle" className="wb-disclosure-icon" /><div><span className="tm-kicker">额度门槛</span><h2>工作区设置</h2><p>管理员调整单次调用预留和工作区月额度。</p></div><span className="tm-account-summary">展开设置</span></summary>{isAdmin ? <div className="tm-admin-grid"><form onSubmit={updateSettings}><label>单次调用预留<input type="number" min="1" max="1000000" value={reservationTokens} onChange={(event) => setReservationTokens(event.target.value)} /><small>1 至 1,000,000 tokens</small></label><button className="tm-button tm-button-primary" disabled={busy === 'settings'}>{busy === 'settings' ? '保存中…' : '保存调度设置'}</button></form><div><label>工作区月额度<input value={workspaceLimit} onChange={(event) => setWorkspaceLimit(event.target.value)} placeholder="留空表示不限额" inputMode="numeric" /></label><button className="tm-button tm-button-secondary" disabled={Boolean(busy)} onClick={() => void updateQuota('workspace', 'all', workspaceLimit)}>保存工作区额度</button></div></div> : <p className="tm-readonly">额度及成员权限由管理员维护。</p>}</details>}
      <details className="tm-card tm-account"><summary className="tm-card-head"><Icon name="triangle" className="wb-disclosure-icon" /><div><span className="tm-kicker">账号</span><h2>修改密码</h2><p>修改成功后需要重新登录。</p></div><span className="tm-account-summary">展开</span></summary><form className="tm-password-form" onSubmit={changePassword}><label>当前密码<input required type="password" value={passwords.current} onChange={(event) => setPasswords({ ...passwords, current: event.target.value })} /></label><label>新密码<input required minLength={12} type="password" value={passwords.next} onChange={(event) => setPasswords({ ...passwords, next: event.target.value })} /></label><button className="tm-button tm-button-primary" disabled={busy === 'password'}>{busy === 'password' ? '保存中…' : '修改密码'}</button></form></details>
...
function MemberRow({ member, projects, isAdmin, busy, onUpdate, onProjects, onQuota }: { member: TeamMember; projects: TeamProject[]; isAdmin: boolean; busy: string | null; onUpdate: (member: TeamMember, role: 'admin' | 'member', active: boolean, password: string) => Promise<void>; onProjects: (member: TeamMember, ids: string[]) => Promise<void>; onQuota: (scope: 'member', id: string, value: string) => Promise<void> }) {
  ...
  return <article className={`tm-member-row ${!member.active ? 'is-disabled' : ''}`}>
    ...
    {isAdmin && open && <div className="tm-member-controls">
      ...
      {<><label>月额度<input value={limit} placeholder="不限额" inputMode="numeric" onChange={(event) => setLimit(event.target.value)} /></label>
