...
function TeamPage({ csrfToken, onUnauthorized, user }: PageProps) {
  ...
  return <div className="tm-page">
    <PageHeader title="团队与额度" description="查看成员用量，分配项目权限，为团队设置每月额度。" actions={<button className="tm-button tm-button-secondary" onClick={() => void load()} disabled={loading}>{loading ? '更新中…' : '刷新'}</button>} />
    ...
    {data && <>
      ...
      <details className="tm-method-note"><summary>额度如何计算</summary><p>成员和项目共享运行记录；额度按每月 UTC 重置，统计模型输入与输出 tokens（含缓存输入）。额度是调度门槛，不是美元金额或供应商订阅 credits；单次执行可能超过预留值，待核对占用会保留到管理员核对。</p></details>
...
function MemberRow({ member, projects, isAdmin, busy, onUpdate, onProjects, onQuota }: { member: TeamMember; projects: TeamProject[]; isAdmin: boolean; busy: string | null; onUpdate: (member: TeamMember, role: 'admin' | 'member', active: boolean, password: string) => Promise<void>; onProjects: (member: TeamMember, ids: string[]) => Promise<void>; onQuota: (scope: 'member', id: string, value: string) => Promise<void> }) {
  ...
  return <article className={`tm-member-row ${!member.active ? 'is-disabled' : ''}`}>
    ...
    {isAdmin ? <button className="tm-button tm-button-secondary tm-manage-member" aria-expanded={open} onClick={() => setOpen((value) => !value)}>{open ? '收起管理' : '管理成员'}</button> : <span className="tm-member-readonly">{member.role === 'admin' ? '管理员' : '成员'} · 额度由管理员维护</span>}
