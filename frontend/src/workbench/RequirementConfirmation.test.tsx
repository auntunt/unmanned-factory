...
it('renews analysis without claiming to skip to the next stage', async()=>{
 vi.mocked(request).mockResolvedValue({...run,status:'received'})
 render(<BudgetResume {...props} run={{...run,revision:0,source:{operation:'general'},status:'needs_human'} as unknown as Run} />)
 expect(screen.queryByRole('button',{name:'续跑并进入下一阶段'})).toBeNull()
 fireEvent.click(screen.getByRole('button',{name:'追加额度并重新分析'}))
 await waitFor(()=>expect(props.onChanged).toHaveBeenCalled())
