...
import { CopyValue, runTitle, RunMode, LoadingCard } from './presentation'
...
function EventEvidence({ event }: { event: AuditEvent }) {
  ...
  return <details className="wb-event-evidence"><summary className="wb-event-summary"><Icon name="triangle" className="wb-disclosure-icon" />{event.task_id ? `${event.task_id} · ` : ''}{summary}（展开失败证据）</summary><pre>{payloadText}</pre></details>
}
...
function CheckEvidence({ check, index, context }: { check: Record<string, unknown>; index: number; context?: string }) {
  ...
  return <details className="wb-check-detail"><summary><Icon name="triangle" className="wb-disclosure-icon" /><span>{name}</span><strong className={passed ? 'wb-check-pass' : 'wb-check-fail'}>{outcome}</strong></summary><div className="wb-check-meta"><span>退出码：{check.exit === null || check.exit === undefined ? '—' : String(check.exit)}</span><span>超时：{check.timeout === true ? '是' : '否'}</span>{typeof check.duration_s === 'number' && <span>耗时：{check.duration_s}s</span>}</div>{hasOutput && <div className="wb-check-output">{typeof check.stdout === 'string' && <div><span className="wb-artifact-label">stdout</span><pre>{check.stdout}</pre></div>}{typeof check.stderr === 'string' && <div><span className="wb-artifact-label">stderr</span><pre>{check.stderr}</pre></div>}</div>}</details>
}

export function ExecutionProgress({ run }: { run: Run }) {
  const live = new Map<string, Record<string, unknown>>()
  const checkpoint = (run as Run & { checkpoint?: { tasks?: unknown[] } }).checkpoint
  for (const source of [run.artifacts?.tasks, checkpoint?.tasks, run.tasks]) {
    if (Array.isArray(source)) for (const task of source) {
  ...
  return <section className="wb-execution-card" aria-label="任务实时进度"><h2>{run.execution_mode === 'continuous' ? '持续编码进度' : '任务实时进度'}</h2><p>{run.execution_mode === 'continuous' ? '执行助手持续修改、运行和修正，六环按实际活动更新。' : '独立任务并行推进；失败只阻塞依赖它的分支。本地命令与检查排队执行。'}</p><div className="wb-attempt-groups">{(run.plan?.tasks ?? []).map(task => {
    ...
    const dependencies = (task.depends_on ?? []).filter(id => !['verified', 'completed'].includes(String(live.get(id)?.status)))
    const text = status === 'running' ? labels[String(activity.phase)] ?? '正在执行' : ['pending', 'queued', 'blocked'].includes(status) ? dependencies.length ? `等待前置任务：${dependencies.join('、')}` : '等待调度' : statusLabel(status)
    return <article className="wb-attempt-group" key={task.id}><header><strong>{task.title}</strong><span role="status">{text}</span></header></article>
  })}</div></section>
}
...
function PolicySnapshot({ run, isAdmin }: { run: Run; isAdmin: boolean }) {
  ...
  if (!policy) return <section><h3>执行策略</h3><p className="wb-runtime-note">服务端没有返回本次运行的策略冻结快照；不会用当前项目设置冒充历史策略。</p></section>
  return <section><h3>执行策略</h3><div className="wb-policy-snapshot"><strong>{policy.mode === 'autonomous' ? '自动模式' : '监督模式'} · 策略 v{policy.revision}</strong><p>{policy.mode === 'autonomous' ? '符合冻结范围的计划由后端自动授权；超出范围时会保留实际判断依据。' : '可到项目自动设置复核并接续等待计划；已执行的计划版本和证据会保留。'}</p>{isAdmin && <Link className="wb-text-link" to={automationHref}>复核项目自动设置</Link>}</div></section>
}
  ...
  return (
    <section className="wb-detail-card" aria-labelledby="delivery-title">
      ...
      <div className="wb-artifact-list">
        <div className="wb-artifact"><span className="wb-artifact-label">基线 SHA</span><span className="wb-sha">{typeof baseSha === 'string' ? <CopyValue value={baseSha} label="基线 SHA" length={7} /> : '—'}</span></div>
        <div className="wb-artifact"><span className="wb-artifact-label">交付 commit SHA</span><span className="wb-sha">{typeof commit === 'string' ? <CopyValue value={commit} label="提交 SHA" length={7} /> : '—'}</span></div>
        {artifacts?.publication_type === 'initial' ? <div className="wb-artifact"><span className="wb-artifact-label">首次上传{typeof artifacts.published_branch === 'string' && artifacts.published_branch ? ` · ${artifacts.published_branch}` : ''}</span>{safeRepositoryUrl(artifacts.repository_url) ? <a href={safeRepositoryUrl(artifacts.repository_url)!} target="_blank" rel="noreferrer">在 GitHub 打开仓库 <Icon name="external" /></a> : <span>已记录首次上传</span>}</div> : <div className="wb-artifact"><span className="wb-artifact-label">来源 PR</span>{safeGithubPr(prUrl) ? <a href={prUrl} target="_blank" rel="noreferrer">在 GitHub 打开 PR <Icon name="external" /></a> : <span>{typeof prUrl === 'string' && prUrl ? '已记录链接，但不是受支持的 GitHub PR 地址' : '尚未生成 PR'}</span>}</div>}
...
export function PlanPanel({ run, showTasks = true }: { run: Run; showTasks?: boolean }) {
  const [view, setView] = useState<'graph' | 'list'>('graph')
  const statuses = useMemo(() => taskStatusMap(run.tasks), [run.tasks])
  const tasks = useMemo<PlanTask[]>(() => (run.plan?.tasks ?? []).map((task) => ({ ...task, depends_on: task.depends_on ?? [], paths: task.paths ?? [], acceptance: task.acceptance ?? [], status: statuses.get(task.id) ?? task.status })), [run.plan?.tasks, statuses])
...
function VerificationEvidence({ run }: { run: Run }) {
  ...
  const ledger = run.artifacts?.acceptance_ledger as { total?: number; counts?: { pass?: number; fail?: number; unverified?: number }; items?: Array<{ id: string; text: string; status: string; evidence: string; spec_path?: string }> } | undefined
  const rows = Array.isArray(ledger?.items) ? ledger.items : []
  if (!checks.length && !failure && !rows.length) return <section className="wb-detail-card"><h2>检查证据</h2><p className="wb-runtime-note">{run.status === 'verifying' ? '正在运行检查，完成后结果会显示在这里。' : '尚未记录可展示的检查结果。'}</p></section>
  return <section className="wb-detail-card" aria-labelledby="verification-title"><h2 id="verification-title">检查证据</h2>{rows.length > 0 && <><p>逐项验收：通过 {ledger?.counts?.pass ?? 0} · 失败 {ledger?.counts?.fail ?? 0} · 未验证 {ledger?.counts?.unverified ?? 0}</p><div className="wb-checks">{rows.map((row) => <details className="wb-check-detail" key={row.id}><summary><Icon name="triangle" className="wb-disclosure-icon" /><span>{row.text}</span><strong className={row.status === 'pass' ? 'wb-check-pass' : 'wb-check-fail'}>{({ pass: '通过', fail: '失败', unverified: '未验证' } as Record<string, string>)[row.status] ?? '未验证'}</strong></summary><p>{row.evidence || '尚无对应证据'}</p>{row.spec_path && run.project_id !== null && <SpecEvidenceLink projectId={run.project_id} path={row.spec_path} runId={run.id}/>}</details>)}</div></>}<p className="wb-runtime-note">{verificationScopeNote(run)}</p>{failure && <details className="wb-check-detail"><summary><Icon name="triangle" className="wb-disclosure-icon" /><span>运行记录的失败原因</span><strong className="wb-check-fail">展开证据</strong></summary><pre>{failure}</pre></details>}{checks.length > 0 && <div className="wb-checks">{checks.map(({ check, context }, index) => <CheckEvidence check={check} context={context} index={index} key={`${context}-${String(check.name ?? 'check')}-${index}`} />)}</div>}</section>
}
...
export default function RunPage({ csrfToken, onUnauthorized, user }: RunPageProps) {
  ...
  const effectiveView = runView(searchParams.get('view'), run ? runGuidance(run).view : 'requirements')
  const pollEvents = effectiveView === 'execution' || tab === 'events'
  useEffect(() => {
    const sequence = auxiliarySequence.current + 1
    auxiliarySequence.current = sequence
    if (tab === null && !pollEvents) return
    setAuxError(null)
    eventCursor.current = pollEvents ? 0 : eventCursor.current
    if (tab === 'conversation') setMessages(null)
    if (pollEvents) setEvents(null)
    const controller = new AbortController()
    const load = () => { if (tab === 'conversation') void loadConversation(controller.signal, sequence); if (pollEvents) void loadEvents(controller.signal, sequence) }
    load()
    const timer = window.setInterval(load, 3500)
    return () => { controller.abort(); window.clearInterval(timer) }
  }, [loadConversation, loadEvents, tab, runId, pollEvents])

  useEffect(() => subscribeDataRefresh(() => { void loadRun() }), [loadRun])

  const act = async (kind: 'clarify' | 'continue' | 'approve' | 'cancel' | 'discard' | 'publish') => {
    if (!run || !runId || busy !== null) return
  ...
  const distill = async (event: FormEvent) => {
    ...
    try { const candidate = await request<{ id: string; name: string }>('/api/v3/runs/' + encodeURIComponent(runId) + '/distill', { method: 'POST', csrfToken, onUnauthorized, body: { name: distillName.trim() } }); setDistillNotice(`已生成草稿能力“${candidate.name}”，请在能力库审阅后配置。`); setDistillName('') } catch (cause) { if (!(cause instanceof WorkspaceApiError && cause.status === 401)) setError(errorText(cause)) } finally { setBusy(null) }
  }

  if (!runId) return <div className="wb-detail-page wb-run-page"><EmptyState title="缺少运行编号" description="请从运行列表打开一个有效的运行。" /></div>
  if (loading && !run) return <div className="wb-detail-page wb-run-page"><LoadingCard label="正在读取运行" /></div>
  ...
  const canActOnRun = !run.retry_run_id && (isAdmin || (actorId !== undefined && actorId !== null && String(actorId) === String(user?.id)))
  ...
  const canPublish = isAdmin && run.status === 'ready_for_review' && !run.source?.skill_ingestion_id
  ...
  const canDistill = !isIngestion && isAdmin && ['ready_for_review', 'published'].includes(run.status)
  const guidance = runGuidance(run)
  const activeView = runView(searchParams.get('view'), guidance.view)
  const projectHref = run.source?.target_agent_id ? `/agents/${encodeURIComponent(String(run.source.target_agent_id))}` : `/projects/${encodeURIComponent(String(run.project_id))}?stage=${guidance.stage}`
  ...
  return <div className="wb-detail-page wb-run-page"><PageHeader title={<>{runTitle(run)}<RunMode run={run} /></>} description={`需求运行 · 更新于 ${formatDate(run.updated_at)}`} actions={<RunActions run={run} canClarify={canClarify} canDiscard={canDiscard} canCancel={canCancel} busy={busy !== null} onDiscard={() => void act('discard')} onCancel={() => void act('cancel')} />} />
    ...
    <p className="wb-run-breadcrumb"><Link to={projectHref}>{run.project_id ? '返回项目闭环' : '返回职能体'}</Link><span>·</span><Link to="/runs">运行看板</Link></p>
    ...
    {typeof run.source?.skill_ingestion_id === 'string' && <section className="wb-card"><h2>职能包适配结果</h2><p>独立验收通过后需要管理员核对映射并签署，能力才会生效。</p><Link className="wb-button wb-button-primary" to={run.source.target_agent_id ? `/agents/${encodeURIComponent(String(run.source.target_agent_id))}` : `/agents?project=${encodeURIComponent(String(run.project_id))}&ingestion=${encodeURIComponent(run.source.skill_ingestion_id)}`}>查看映射与人签评审</Link></section>}
    ...
    <div className="wb-detail-grid"><div className="wb-detail-main"><section className="wb-detail-card" role="tabpanel" id="run-view-panel" aria-labelledby={`run-tab-${activeView}`}><div className="wb-detail-meta"><span>状态：<StatusBadge status={runDisplayStatus(run)} /></span><span>当前计划版本：<strong>{run.revision}</strong></span><span>创建于：<strong>{formatDate(run.created_at)}</strong></span></div>
      {activeView === 'requirements' && <><div className="wb-detail-kicker">原始需求</div><h2>需求与澄清</h2><p className="wb-message-content">{run.request}</p>{run.project_id !== null && <RunSpecReferences source={run.source} projectId={run.project_id}/>}{run.triage?.reasons?.length ? <details><summary><Icon name="triangle" className="wb-disclosure-icon" />规划时的判断（历史记录）</summary><p className="wb-runtime-note">{run.triage.reasons.join('；')}</p></details> : null}{questions.length > 0 && <><h3>需要补充的事实</h3><ul>{questions.map((question) => <li key={question}>{question}</li>)}</ul></>}{canClarify && <div className="wb-field"><label htmlFor="run-clarification">{questions.length ? `补充要求并生成下一版计划（当前 v${run.revision}）` : '补充目标、范围或验收标准并生成下一版计划'}</label><textarea id="run-clarification" value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder={questions.length ? '提供事实、边界或选择，提交后会生成下一版计划。' : '当前没有可展示的具体问题；请补充需要系统据以规划的事实。'} /></div>}{canClarify && <div className="wb-form-actions"><button className="wb-button wb-button-primary" disabled={!answer.trim() || busy !== null} onClick={() => void act('clarify')}>{busy === 'clarify' ? '正在生成下一版计划…' : '补充要求并生成下一版计划'}</button></div>}{!canClarify && guidance.kind !== 'requirements' && <p className="wb-runtime-note">当前无需补充需求。请根据上方状态指引进入对应工作视图。</p>}</>}
      ...
      {activeView === 'execution' && <>{!run.retry_run_id && ['needs_human', 'failed', 'cancelled'].includes(run.status) && <section className="wb-policy-snapshot" id="run-recovery" aria-labelledby="run-recovery-title"><h2 id="run-recovery-title">自动处理未完成</h2>{executionFailure(run)?.startsWith('out-of-scope changes:') ? <><p>这次修改涉及当前任务范围以外的文件。回答后，助手会按原任务范围修正草稿并继续；如果确实要改变任务范围，可另行修改计划。</p><p>涉及文件：{executionFailure(run)?.slice('out-of-scope changes:'.length)}</p></> : executionFailure(run)?.startsWith('worker changed test/check infrastructure:') ? <p>这次改动包含测试配置。是否保留正常的项目依赖信息，并恢复原有测试配置后继续？你可以直接回答，助手会在当前草稿上处理。</p> : <p>自动处理已尝试但尚未完成。你无需编写处理指令；可以查看原因，或直接重试。原计划和已有成果会保留。</p>}{canContinue ? <form onSubmit={(event) => { event.preventDefault(); if (busy === null) void act('continue') }}><details><summary><Icon name="triangle" className="wb-disclosure-icon" />补充信息（可选）</summary><div className="wb-field"><label htmlFor="run-recovery-answer">补充信息（可选）</label><textarea id="run-recovery-answer" value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="只有需要补充业务信息时再填写。" /></div></details><p>失败证据会自动交给执行助手，补充信息可留空，不重新规划。已完成的任务不会重新调用模型；未完成的草稿会继续修正并验证。</p><button className="wb-button wb-button-primary" disabled={busy !== null}>{busy === 'continue' ? '正在继续…' : '重试自动处理'}</button>{error && <ErrorNotice message={error} />}</form> : canActOnRun ? <p>当前没有可直接恢复的执行现场，请通过“补充要求并生成下一版计划”或下方新运行入口处理。</p> : <p>只有任务发起人或管理员可以处理这次运行，请联系他们继续。</p>}{canRetry && <details><summary><Icon name="triangle" className="wb-disclosure-icon" />按当前配置创建新运行</summary><p>保留本次记录，使用当前项目配置重新规划。职能体任务沿用原职能体版本与模型快照。相同问题未处理时可能再次失败。</p><button className="wb-button" disabled={busy !== null} onClick={() => void retry()}>{busy === 'retry' ? '正在创建…' : '创建新运行'}</button></details>}{isAdmin && <p><Link to="/settings/runtime">查看运行配置</Link></p>}</section>}<ExecutionLog key={String(run.id)} events={events} error={auxError} /><ExecutionAttempts run={run} /><PlanPanel run={run} /></> }
      {activeView === 'verification' && <><OperationResults run={run} /><VerificationEvidence run={run} /></>}
      {activeView === 'delivery' && <><Deliverables run={run} csrfToken={csrfToken} onUnauthorized={onUnauthorized} isAdmin={isAdmin} onPublish={canPublish ? () => void act('publish') : undefined} publishing={busy === 'publish'} /><DeliveryEvidence run={run} />{canDistill && <section className="wb-detail-card wb-distill-card"><div><span className="wb-eyebrow">能力生产</span><h2>能力归档</h2><p>交付完成后系统会自动留下可追溯的能力草稿；你也可手动生成一个额外草稿供比较或补充。</p></div>{(() => { const candidateId = (run as Run & { capability_candidate_id?: unknown }).capability_candidate_id ?? run.artifacts?.capability_candidate_id; return typeof candidateId === 'string' && candidateId ? <div className="wb-notice">本次交付已归档为能力草稿。<Link className="wb-text-link" to={`/capabilities?selected=${encodeURIComponent(candidateId)}`}>打开已归档能力</Link></div> : null })()}<details className="wb-distill-manual"><summary><Icon name="triangle" className="wb-disclosure-icon" />手动生成额外草稿</summary><form onSubmit={distill} className="wb-distill-form"><input required maxLength={120} value={distillName} onChange={(event) => setDistillName(event.target.value)} placeholder="候选能力名称" /><button className="wb-button wb-button-primary" disabled={busy !== null}>{busy === 'distill' ? '提炼中…' : '生成额外草稿'}</button></form></details>{distillNotice && <div className="wb-notice">{distillNotice} <Link className="wb-text-link" to="/capabilities">打开能力库</Link></div>}</section>}</>}
    </section><details className="wb-detail-card"><summary><Icon name="triangle" className="wb-disclosure-icon" /><strong>对话和审计记录</strong></summary><div className="wb-detail-tabs" role="tablist" aria-label="运行记录"><button className={`wb-tab ${tab === 'conversation' ? 'wb-tab-active' : ''}`} role="tab" aria-selected={tab === 'conversation'} onClick={() => setTab('conversation')}>对话</button><button className={`wb-tab ${tab === 'events' ? 'wb-tab-active' : ''}`} role="tab" aria-selected={tab === 'events'} onClick={() => setTab('events')}>审计事件</button></div>{tab === null ? <p className="wb-runtime-note">按需打开对话或审计事件，避免在不查看时重复读取记录。</p> : tab === 'conversation' ? <ConversationPanel messages={messages} loading={loading} error={auxError} /> : <EventsPanel events={events} loading={loading} error={auxError} />}</details>{isAdmin && !isIngestion && <RunKnowledge key={`${run.id}:${run.revision}`} runId={run.id} revision={run.revision} status={run.status} artifacts={run.artifacts} csrfToken={csrfToken} onUnauthorized={onUnauthorized} />}</div><aside className="wb-detail-rail" aria-label="运行详情摘要"><section className="wb-detail-card"><h2>运行概览</h2><div className="wb-side-stat"><span>运行 ID</span><CopyValue value={run.id} label="运行编号" /></div><div className="wb-side-stat"><span>工程 ID</span><CopyValue value={run.project_id || '职能体范围（无项目）'} label="工程编号" /></div><div className="wb-side-stat"><span>状态</span><span><StatusBadge status={runDisplayStatus(run)} /></span></div><div className="wb-side-stat"><span>当前指引</span><span>{guidance.label}</span></div><div className="wb-side-stat"><span>风险</span><span>{run.triage?.risk ? { low: '低风险', medium: '中风险', high: '高风险' }[run.triage.risk] : '—'}</span></div></section><details className="wb-detail-card"><summary><Icon name="triangle" className="wb-disclosure-icon" /><strong>运行配置快照</strong></summary><p>冻结＝提交时定格，之后的修改不影响本次运行。</p><section><h3>已搭配能力</h3>{((run as Run & { module_snapshot?: { id: string; name: string; version: number }[] }).module_snapshot ?? []).length ? <ul>{(run as Run & { module_snapshot: { id: string; name: string; version: number }[] }).module_snapshot.map(m => <li key={m.id}>{m.name} · v{m.version}</li>)}</ul> : <p className="wb-runtime-note">使用平台通用能力与本次职能体配置</p>}<Link className="wb-text-link" to={run.project_id === null ? projectHref : `/projects/${encodeURIComponent(String(run.project_id))}?tab=agent`}>搭配后续任务的能力</Link></section><PolicySnapshot run={run} isAdmin={isAdmin} /><FrozenRuntime run={run} /></details></aside></div></div>
}
