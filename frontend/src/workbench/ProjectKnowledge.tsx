...
export default function ProjectKnowledge({ projectId, csrfToken, onUnauthorized }: PageProps & { projectId: string | number }) {
  ...
  return <div className="pk-workspace">
    ...
    {skipped.length > 0 && <details className="pk-section"><summary>无需沉淀 · {skipped.length} 条</summary>{skipped.map((item) => <article key={item.id}><h3>{item.title}</h3><p className="pk-text">{item.disposition?.source?.content || item.content}</p></article>)}</details>}
  </div>
}
