...
const blankDraft: ProjectDraft = { name: '', repository: '', workspace: '', base_branch: 'main', checks: {}, auto_issues: false, auto_publish: false, budget_usd: '10' }
...
function ProjectForm({ csrfToken, onUnauthorized, onCreated, onCancel }: PageProps & { onCreated: (project: ProjectRecord) => void; onCancel: () => void }) {
  ...
  return <section className="wb-card wb-create-card" aria-labelledby="create-project-title">
    ...
    <form className="wb-form" onSubmit={submit}>
      ...
      <details className="wb-advanced"><summary>检查与自动化设置</summary><div className="wb-advanced-body"><ChecksEditor value={draft.checks} onChange={(checks) => update('checks', checks)} /><div className="wb-check-options"><label className="wb-checkbox"><input type="checkbox" checked={draft.auto_issues} onChange={(event) => update('auto_issues', event.target.checked)} />允许带 factory-ready 标签的问题自动执行</label><label className="wb-checkbox"><input type="checkbox" checked={draft.auto_publish} onChange={(event) => update('auto_publish', event.target.checked)} />允许通过验证后自动交付</label></div></div></details>
...
export default function ProjectsPage({ csrfToken, onUnauthorized }: PageProps) {
  ...
  useEffect(() => {
    const controller = new AbortController(); setProjects(null); setError(null)
  ...
  return <div className="wb-page">
    <PageHeader title="项目" description="管理工程上下文、检查规则和需求入口。" actions={<button className="wb-button wb-button-primary" onClick={() => setCreateOpen((value) => !value)}>{createOpen ? '关闭登记' : '登记项目'} <span aria-hidden="true">＋</span></button>} />
    {createOpen && <ProjectForm csrfToken={csrfToken} onUnauthorized={onUnauthorized} onCancel={() => setCreateOpen(false)} onCreated={(project) => { setProjects((current) => current ? [project, ...current] : [project]); setCreateOpen(false); void navigate(`/projects/${encodeURIComponent(String(project.id))}`) }} />}
    {error && <ErrorNotice message={error} />}
    {projects === null && !error && <div className="wb-card"><div className="wb-list-placeholder"><span /><span /><span /></div></div>}
    {projects && projects.length === 0 && <div className="wb-card"><EmptyState title="还没有项目" description="登记一个已经存在的 Git 工作区，开始接收工程需求。" action={<button className="wb-button wb-button-primary" onClick={() => setCreateOpen(true)}>登记第一个项目</button>} /></div>}
