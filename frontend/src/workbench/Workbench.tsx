...
const navigation = [
  { to: '/', label: '开始制作', icon: 'overview' as const, end: true },
  { to: '/projects', label: '我的作品', icon: 'project' as const, end: false },
]
const managementNavigation = [
  { to: '/overview', label: '工作总览', icon: 'overview' as const, end: true },
  { to: '/runs', label: '运行记录', icon: 'runs' as const, end: false },
    ...
    <div className="wb-shell">
      ...
      <aside className={`wb-sidebar ${navOpen ? 'is-open' : ''}`} aria-label="工作台导航">
        ...
        <nav className="wb-nav" aria-label="主导航">
          ...
          {navigation.map(item => <NavLink key={item.to} aria-label={item.label} to={item.to} end={item.end} className={({ isActive }) => `wb-nav-item ${isActive ? 'is-active' : ''}`}><span className="wb-nav-icon"><Icon name={item.icon} /></span><span>{item.label}</span></NavLink>)}
          <details open={[...managementNavigation, ...visibleSecondary].some(item => location.pathname.startsWith(item.to))}><summary className="wb-nav-item">管理</summary>
          {[...managementNavigation, ...visibleSecondary].map(item => <NavLink key={item.to} aria-label={item.label} to={item.to} end={item.end} className={({ isActive }) => `wb-nav-item ${isActive ? 'is-active' : ''}`}><span className="wb-nav-icon"><Icon name={item.icon} /></span><span>{item.label}</span></NavLink>)}
          </details>
        </nav>
