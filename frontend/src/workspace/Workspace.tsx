...
function RunDetail({ run, project, csrfToken, onChanged, onUnauthorized }: { run: Run; project?: Project; csrfToken: string; onChanged: (run: Run) => void; onUnauthorized: () => void }) {
...
export default function Workspace({ user, csrfToken, onLogout }: WorkspaceProps) {
  ...
  const loadRuns = useCallback(async () => {
    ...
    try {
      ...
      setSyncError(null)
      setSelectedRunId((current) => current && response.runs.some((run) => String(run.id) === String(current)) ? current : response.runs[0]?.id ?? null)
