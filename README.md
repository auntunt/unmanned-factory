...
webuddy 是可定制的 AI 工作伙伴：员工在工作台用自然语言描述目标，由职能体完成规划、执行、验证与成果交付，并通过 Skill 和维护对话持续积累专门能力。目标方向是与底层 Agent 无关的企业 AI 工作管控平台——管理者按组织范围查看人员、Agent、项目、预算、审批和风险；目前已实现的是组织树、项目归属与负责人的**只读**管理视图（`/management`），预算分配、审批流、委派与 Agent 治理仍是后续阶段，尚未实现。

[企业组织权限与领导控制台设计](docs/design/enterprise-governance-console.md)记录目标定位、递归组织模型、授权继承、双工作面与渐进实施边界（设计方向，大部分未实现）；已实现部分见 [企业治理 v1 契约](docs/implementation/enterprise-governance-v1/CONTRACT.md)。

界面更新：[2026-09-14 工作台改版与实际截图](docs/uiux-2026-09-14.md)。项目与总览共用六阶段条，记录全宽展开；运行动作分级，巡检与服务器配置移到项目设置。文档中的旧环形图截图仅代表历史版本，本页链接含最新隔离演练截图与 axe 基线。
...
运维维护的 Python 仓库若有 `Dockerfile.test`（优先）或 `Dockerfile`，探测会建议容器内 `pytest`，并只收集 `tests/`（或 `test/`）目录。采纳后，执行主机需要可访问的 Docker daemon；系统以 `docker build --network=none` 构建项目镜像，再在无网络、只读源代码挂载、非 root 用户的容器里将源码复制到容器内临时目录执行测试，**不会修改宿主机工作区，也不会退回宿主机 Python**。镜像必须预装测试依赖；生产镜像不含 pytest 时，请在仓库提供 `Dockerfile.test`。构建时也禁用网络，因此基础镜像与依赖须已缓存或在镜像中备齐。仓库中原有的宿主机 `pytest` 配置不会被静默改写；在“运维维护 → 代码库 → 建议检查”采纳容器建议后才切换。
