...
全绿自动落地（`merged`）的任务不算积压 —— 那是这套系统的**常态**，算进去的话工厂
跑得越顺，积压数越大，那个读数就没人会看。只有最后一轮判决是 `escalated`（机器
交不出结论、上人了）且还没人定案的才算。被打回（`reworked`）也不算：worker 会
自己重跑，没人在等。

积压除了个数还报**等了多久**和**谁在等**：

```
积压里等最久    : T-积压 已等 2.0d（这段不算人时 —— 人还没来看）
```

等三天和等一分钟在一个「有 1 个在等」上完全一样，而排优先级要的正是这个差别。
这段等待时长**不进人时**：人还没来看，这段时间没人在花。混进去的话「人时占比」
会随积压一路涨过 100%，而那个数正是用来证明「无人」程度的。
