...
webuddy 是与底层 Agent 无关的企业 AI 工作管控平台。员工在工作台用自然语言描述目标，由职能体完成规划、执行、验证与交付；管理者在控制台按组织范围管理人员、Agent、项目、权限、预算、审批和风险。Codex、Claude Code、模型 API、Skill、工具和验证器通过适配层热插拔，任务、权限与审计不绑定任一执行器。

[企业组织权限与领导控制台设计](docs/design/enterprise-governance-console.md)记录了新的产品定位、递归组织模型、授权继承、双工作面与渐进实施边界。
